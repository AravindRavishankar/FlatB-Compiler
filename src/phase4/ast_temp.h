...
class llvmVisitor : public visitor {
	...
	void *go(AstIdentifier *node)
	{
      ...
      void *p=node->exp->visit(this);
			...
			GlobalVariable *var = new GlobalVariable(*Module_Main, ArrayType::get(Type::getInt64Ty(Context_Main), 100), false, GlobalValue::CommonLinkage, NULL, node->id);
      var->setInitializer(llvm::ConstantAggregateZero::get(llvm::ArrayType::get(llvm::Type::getInt64Ty(Context_Main), 100)));
        	vT[*node]=var;
		}
		else
		{
